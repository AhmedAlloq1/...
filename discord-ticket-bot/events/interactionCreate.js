...
async function handleSlashCommand(interaction) {
  ...
  if (!command) {
    ...
    return safeReply(interaction, {
      embeds: [
        errorEmbed('This command is not recognized right now. Try again later.'),
      ],
...
async function handleClaimButton(interaction) {
  console.log('🔥🔥🔥 CLAIM BUTTON WAS PRESSED 🔥🔥🔥');
...
async function handleConfirmCloseButton(interaction) {
  ...
  if (interaction.user.id !== ticket.creatorId) {
    return safeReply(interaction, {
      embeds: [
        errorEmbed('Only the ticket owner can respond to this request.'),
      ],
      flags: MessageFlags.Ephemeral,
    });
  }
...
async function handleCancelCloseButton(interaction) {
  ...
  return safeReply(interaction, {
    content: 'Close request cancelled — this ticket stays open.',
  });
}
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      await safeReply(interaction, {
        embeds: [
          errorEmbed('Something went wrong handling that action.'),
        ],
        flags: MessageFlags.Ephemeral,
      });
    }
