...
function load() {
  try {
    ...
    console.error('[DATASTORE] Failed to read tickets.json, starting with empty state:', err);
    return { tickets: {} };
  }
}
...
function save(data) {
  try {
    ...
    console.error('[DATASTORE] Failed to save tickets.json:', err);
  }
}
...
function updateTicket(channelId, updates) {
  ...
  state.tickets[channelId] = { ...state.tickets[channelId], ...updates };
  save(state);
  return state.tickets[channelId];
}
